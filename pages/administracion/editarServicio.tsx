...
import { editarServicio } from '../../utils/servicio';
...
const EditarServicio: NextPage = () => {
    ...
    const onEditService = async ({ servicename }: FormData) => {
        ...
        //@ts-ignore[1]
        const { hasError, message } = await editarServicio(router.query.id, servicename, image, show)

        if (hasError) {
            toast.error('No se logro editar el servicio', {
                position: "top-center",
                autoClose: 2000,
                hideProgressBar: false,
                closeOnClick: true,
    ...
    return (
        <>
            <Layout title='Editar Servicio - Taller Gonzales'>
